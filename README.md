Online Voting System

Team Members:
1. Jayanth Amanchy
   Prepared documentation (Phase 1 & Phase 2)
   Organized GitHub repository
3. Abhilaash Gali
   Helped with testing and validation
   Assisted in debugging
4. Shantanu S Shenoy
   Integrated all modules
   Developed voting and authentication logic
5. Hanumath Krishna
   Implemented core Java classes
   Collected outputs and screenshots

Problem Description:
The system is developed using Object-Oriented Programming (OOP) concepts in Java. It simulates a basic voting process including voter login, vote casting, and result calculation.
Main Classes
  Voter.java – Handles voter details and voting actions
  Candidate.java – Stores candidate information and vote count
  Vote.java – Represents a vote and simulates encryption
  VotingSystem.java – Core logic for managing voting operations
  Administrator.java – Manages candidates and election flow
  ElectionOfficer.java – Monitors election status
  Main.java – Entry point of the application
How to Run
  Compile all .java files
  Run Main.java
  Follow console instructions to vote

Sample Input and Output:
  Input:
    Enter Voter ID:
    1
    Enter Password:
    123
    Enter Candidate ID to vote:
    2
  Output:
    Enter Voter ID:
    Enter Password:
    Login Successful!
    Candidates:
    1 - INC (Party A)
    2 - BJP (Party B)
    3 - JJP (Party C)
    Enter Candidate ID to vote:
    Vote stored securely: ENC(2)
    Election Results:
    INC: 0 votes
    BJP: 1 votes
    JJP: 0 votes

Tools/Technologies used:
   Java – Core programming language used to develop the application
   Object-Oriented Programming (OOP) – Used for designing modular and reusable code
   Eclipse IDE – Used for writing, compiling, and testing the Java code
   Git & GitHub – Used for version control and project management
   File System – Used for storing project files and reports


